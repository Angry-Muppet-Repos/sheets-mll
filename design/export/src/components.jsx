...
function Chip({ status, children }) {
  ...
  return (
    <span style={{ background: m.bg, color: m.fg, padding: '4px 10px', borderRadius: 999, fontFamily: 'Jost,sans-serif', fontWeight: 500, fontSize: 10.5, letterSpacing: '0.04em', display: 'inline-flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap' }}>
      <span>{m.icon}</span><span>{children || ({on:'On Track',fair:'Fair',warn:'Caution',over:'Over'}[status] || status)}</span>
    </span>
  );
}
...
/* ---------- Data row with zebra ---- */
function DataRow({ cols, widths, zebra, last }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: widths.join(' '), background: zebra ? 'var(--pal-zebra, #EEF2EC)' : '#FAF8F2', fontFamily: 'Jost,sans-serif', fontSize: 12.5, color: 'var(--pal-primary, #1C3D2E)', borderBottom: last ? 0 : '1px solid rgba(28,61,46,0.06)' }}>
      {cols.map((c, i) => (
        <div key={i} style={{ padding: '10px 12px', textAlign: c.align || 'left', display: 'flex', alignItems: 'center', justifyContent: c.align === 'right' ? 'flex-end' : (c.align === 'center' ? 'center' : 'flex-start'), gap: 8, fontVariantNumeric: c.num ? 'tabular-nums' : 'normal', fontWeight: c.bold ? 600 : 400 }}>
          {c.content}
        </div>
      ))}
    </div>
    ...
    <div>
      ...
      <div style={{ background: 'var(--pal-primary, #1C3D2E)', color: '#FAF8F2', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <img src={window.__resources.logoIconCream} alt="" style={{ height: 30, width: 'auto' }} />
          <div>
        ...
        <div style={{ fontFamily: 'Jost,sans-serif', fontSize: 11, fontWeight: 300, color: '#FAF8F2', opacity: 0.78, letterSpacing: '0.04em' }}>
          The Foundation v2 &nbsp;·&nbsp; {tab}
        </div>
      </div>
      {/* Row 2: sub-band */}
      <div style={{ background: 'var(--pal-mid, #2D5C45)', height: 22 }}></div>
      {/* Row 3: accent rule */}
...
function SheetFooter({ note }) {
  return (
    <div style={{ background: 'var(--pal-primary, #1C3D2E)', color: '#FAF8F2', padding: '11px 24px', fontFamily: 'Jost,sans-serif', fontSize: 10.5, fontWeight: 300, letterSpacing: '0.04em', textAlign: 'center', opacity: 0.95 }}>
      The Foundation v2.0 &nbsp;·&nbsp; columnandco.com {note ? ' · ' + note : ''}
    </div>
  );
}

Object.assign(window, { SectionLabel, KpiCard, Chip, Progress, fmt, pct, ColHeader, DataRow, SheetHeader, SheetFooter });
